<template>
	<div class="content">
		<div class="container_of_slide" v-for="(item, index) in NowMessageList" :key="index">
			...
			>
				<div class="now-message-info" :style="{ width: Screen_width + 'px' }">
					<div class="imgInfo" @tap="recover(index)"><image :src="item.img"></image></div>
				...
				<div class="group-btn">
					<div class="top" @tap="top(index)" v-if="item.top">取消置顶</div>
					<div class="top" @tap="top(index)" v-else>置顶</div>
					<div class="removeM" @tap="removeM(index)">删除</div>
				</div>
